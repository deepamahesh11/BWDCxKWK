<script>
    ...
    let deepaIsVisible = $state(false);

    const options = {
        threshold: [0.85, 0.95],
    };

    const DeepaSimpleCallback = (entries, observer) => {
    ...
    const DeepaShowDuckCallback = (entries, observer) => {
        entries.forEach((entry) => {
            const elem = entry.target;
    ...
    const DeepaRemoveDuckCallback = (entries, observer) => {
        entries.forEach((entry) => {
            const elem = entry.target;

            if (entry.intersectionRatio >= 0.9) {
                elem.style.backgroundColor = "#e3ff00";
                deepaIsVisible = false;
            } else if (entry.intersectionRatio < 0.9) {
                elem.style.backgroundColor = "#888888";
            }
        });
...
<div>
    <Scroller layout="left">
        ...
        {#snippet scrolly()}
            <ObservedArticleText callback={DeepaRemoveDuckCallback} {options}>
            ...
            <ObservedArticleText callback={DeepaShowDuckCallback} {options}>
            ...
            <ObservedArticleText callback={DeepaSimpleCallback} {options}>
                Pretty slick, huh?
                <br /><br />
                🤔
                <strong
